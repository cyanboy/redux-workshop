...
Put the `FilterList` component in a new file and let this component be a child of the `Map` component. Create a new component, `Feed` and put the `FilterList` and the `TweetFeed` component inside this "wrapper" component. Make sure you fix the routing, so that it routes to the `Feed` component instead of the `FilterList` component.
